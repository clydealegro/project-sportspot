...
abstract class BaseListing extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array MapInfo[] Collection to store aggregation of MapInfo objects.
	 */
	protected $collMapInfos;

	/**
	 * @var        Criteria The criteria used to select the current contents of collMapInfos.
	 */
	private $lastMapInfoCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collMapInfos = null;
			$this->lastMapInfoCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collMapInfos !== null) {
				foreach ($this->collMapInfos as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getMapInfos() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMapInfo($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collMapInfos collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addMapInfos()
	 */
	public function clearMapInfos()
	{
		$this->collMapInfos = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collMapInfos collection (array).
	 *
	 * By default this just sets the collMapInfos collection to an empty array (like clearcollMapInfos());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initMapInfos()
	{
		$this->collMapInfos = array();
	}

	/**
	 * Gets an array of MapInfo objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Listing has previously been saved, it will retrieve
	 * related MapInfos from storage. If this Listing is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getMapInfos($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ListingPeer::DATABASE_NAME);
		}
		...
		if ($this->collMapInfos === null) {
			if ($this->isNew()) {
			   $this->collMapInfos = array();
				...
				$criteria->add(MapInfoPeer::LISTING_ID, $this->listing_id);

				MapInfoPeer::addSelectColumns($criteria);
				$this->collMapInfos = MapInfoPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				MapInfoPeer::addSelectColumns($criteria);
				if (!isset($this->lastMapInfoCriteria) || !$this->lastMapInfoCriteria->equals($criteria)) {
					$this->collMapInfos = MapInfoPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastMapInfoCriteria = $criteria;
		return $this->collMapInfos;
	}

	/**
	 * Returns the number of related MapInfo objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related MapInfo objects.
	 * @throws     PropelException
	 */
	public function countMapInfos(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ListingPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collMapInfos === null) {
			if ($this->isNew()) {
				...
				$count = MapInfoPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a MapInfo object to this object
	 * through the MapInfo foreign key attribute.
	 *
	 * @param      MapInfo $l MapInfo
	 * @return     void
	 * @throws     PropelException
	 */
	public function addMapInfo(MapInfo $l)
	{
		if ($this->collMapInfos === null) {
			$this->initMapInfos();
		}
		if (!in_array($l, $this->collMapInfos, true)) { // only add it if the **same** object is not already associated
			array_push($this->collMapInfos, $l);
			$l->setListing($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collMapInfos) {
				foreach ((array) $this->collMapInfos as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
