...
abstract class BaseAvailability extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the opening_time field.
	 * @var        string
	 */
	protected $opening_time;

	/**
	 * The value for the closing_time field.
	 * @var        string
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [opening_time] column value.
	 * 
	 *
	 ...
	 */
	public function getOpeningTime($format = 'H:i:s')
	{
		...
		try {
			$dt = new DateTime($this->opening_time);
		} catch (Exception $x) {
			throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->opening_time, true), $x);
		}
	...
	/**
	 * Get the [optionally formatted] temporal [closing_time] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getClosingTime($format = 'H:i:s')
	{
		...
		try {
			$dt = new DateTime($this->closing_time);
		} catch (Exception $x) {
			throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->closing_time, true), $x);
		}
	 ...
	 * @return     Availability The current object (for fluent API support)
	 */
	public function setAvailabilityId($v)
	{
		...
		if ($this->availability_id !== $v) {
			...
			$this->modifiedColumns[] = AvailabilityPeer::AVAILABILITY_ID;
		}
	...
	public function setDay($v)
	{
		...
		if ($this->day !== $v) {
			$this->day = $v;
			$this->modifiedColumns[] = AvailabilityPeer::DAY;
		}
	...
	/**
	 * Sets the value of [opening_time] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Availability The current object (for fluent API support)
	 */
	public function setOpeningTime($v)
	{
		...
		if ( $this->opening_time !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->opening_time !== null && $tmpDt = new DateTime($this->opening_time)) ? $tmpDt->format('H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->opening_time = ($dt ? $dt->format('H:i:s') : null);
				$this->modifiedColumns[] = AvailabilityPeer::OPENING_TIME;
			}
	...
	/**
	 * Sets the value of [closing_time] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Availability The current object (for fluent API support)
	 */
	public function setClosingTime($v)
	{
		// we treat '' as NULL for temporal objects because DateTime('') == DateTime('now')
		// -- which is unexpected, to say the least.
		if ($v === null || $v === '') {
		...
		if ( $this->closing_time !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->closing_time !== null && $tmpDt = new DateTime($this->closing_time)) ? $tmpDt->format('H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->closing_time = ($dt ? $dt->format('H:i:s') : null);
				$this->modifiedColumns[] = AvailabilityPeer::CLOSING_TIME;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->availability_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->day = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->opening_time = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->closing_time = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = AvailabilityPeer::NUM_COLUMNS - AvailabilityPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AvailabilityPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				AvailabilityPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setAvailabilityId($pk);  //[IMV] update autoincrement primary key
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[1] => $this->getDay(),
			$keys[2] => $this->getOpeningTime(),
			$keys[3] => $this->getClosingTime(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setAvailabilityId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setDay($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setOpeningTime($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setClosingTime($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AvailabilityPeer::AVAILABILITY_ID)) $criteria->add(AvailabilityPeer::AVAILABILITY_ID, $this->availability_id);
		if ($this->isColumnModified(AvailabilityPeer::DAY)) $criteria->add(AvailabilityPeer::DAY, $this->day);
		if ($this->isColumnModified(AvailabilityPeer::OPENING_TIME)) $criteria->add(AvailabilityPeer::OPENING_TIME, $this->opening_time);
		if ($this->isColumnModified(AvailabilityPeer::CLOSING_TIME)) $criteria->add(AvailabilityPeer::CLOSING_TIME, $this->closing_time);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AvailabilityPeer::AVAILABILITY_ID, $this->availability_id);
	...
	/**
	 * Generic method to set the primary key (availability_id column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setOpeningTime($this->opening_time);

		$copyObj->setClosingTime($this->closing_time);
	 ...
	 * Otherwise if this Availability has previously been saved, it will retrieve
	 * related ListingToAvailabilitys from storage. If this Availability is new, it will return
		...
		if ($this->collListingToAvailabilitys === null) {
			if ($this->isNew()) {
			   $this->collListingToAvailabilitys = array();
				...
				$criteria->add(ListingToAvailabilityPeer::AVAILABILITY_ID, $this->availability_id);
	 ...
	 */
	public function getListingToAvailabilitysJoinListing($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(AvailabilityPeer::DATABASE_NAME);
		}
		...
		if ($this->collListingToAvailabilitys === null) {
			if ($this->isNew()) {
				...
				$this->collListingToAvailabilitys = ListingToAvailabilityPeer::doSelectJoinListing($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
