...
abstract class BaseCategory extends BaseObject  implements Persistent {
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setCategoryId($pk);  //[IMV] update autoincrement primary key
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CategoryPeer::CATEGORY_ID, $this->category_id);
	 ...
	 * related ListingToCategorys from storage. If this Category is new, it will return
		...
		if ($this->collListingToCategorys === null) {
			if ($this->isNew()) {
			   $this->collListingToCategorys = array();
				...
				$criteria->add(ListingToCategoryPeer::CATEGORY_ID, $this->category_id);
	 ...
	 */
	public function getListingToCategorysJoinListing($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CategoryPeer::DATABASE_NAME);
		}
		...
		if ($this->collListingToCategorys === null) {
			if ($this->isNew()) {
				...
				$this->collListingToCategorys = ListingToCategoryPeer::doSelectJoinListing($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
